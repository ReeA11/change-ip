...
Без аргументов в TTY запускается интерактивный CLI: главный экран показывает свежий network snapshot, все IPv4 с prefix, persistence и health. Это не отдельный fullscreen TUI: ChangeIP временно включает raw input, читает стрелки и ANSI-последовательностями обновляет текущий CLI-блок. Управление: `↑`/`↓` или `k`/`j`, `Enter` — выбор, `Esc` — назад, `q` — выход с главного экрана. Пункт `Language · English` переключает интерфейс на русский; выбор сохраняется в `/etc/change-ip/language` и используется при следующих запусках. По умолчанию используется английский. После apply и rollback состояние перечитывается. При наличии переменной `NO_COLOR` цвета отключаются. Явные команды и non-TTY output сохраняют обычный текстовый интерфейс.
...
Версия release-бинарника берётся из Git-тега автоматически. Менять её вручную в нескольких файлах не нужно. После commit и push создайте и отправьте следующий тег:

```bash
git tag -a v3.0.2 -m "ChangeIP 3.0.2"
git push origin v3.0.2
```

GitHub Actions соберёт amd64/arm64 binaries с версией `3.0.2` и создаст Release. Для следующего выпуска используйте новый тег, например `v3.0.3`. Значение `3.0.2-dev` в `main.go` используется только при локальной сборке без release workflow.
